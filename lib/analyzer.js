...
module.exports = class Analyzer extends Plugin {
  ...
  build() {
    ...
    let targets = uniq(flatten(Object.keys(this._imports).map(fullPath => Object.keys(this._imports[fullPath]))));
    let dependencyTargets = targets.filter(t => this._depFinder.hasDependency(t) && !this._depFinder.isEmberAddon(t));
